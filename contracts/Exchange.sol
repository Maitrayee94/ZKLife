...
contract Exchange is ERC20 {
    ...
    address public IsXTokenAddress;

    // Exchange is inheriting ERC20, because our exchange would keep track of IsX tokens
    constructor(address _IsX) ERC20("InsureX Token", "IsX") {
        require(_IsX != address(0), "Token address passed is a null address");
        IsXTokenAddress = _IsX;
    }

    /**
    * @dev Returns the amount of `IsX` held by the contract
    */
    function getReserve() public view returns (uint) {
        return ERC20(IsXTokenAddress).balanceOf(address(this));
    }
    ...
    */
    function addLiquidity(uint _amount) public payable returns (uint) {
        ...
        uint IsXTokenReserve = getReserve();
        ERC20 IsXToken = ERC20(IsXTokenAddress);
        /*
            If the reserve is empty, intake any user supplied value for
            `Ether` and `Crypto Dev` tokens because there is no ratio currently
        */
        if(IsXTokenReserve == 0) {
            // Transfer the `IsXToken` address from the user's account to the contract
            IsXToken.transferFrom(msg.sender, address(this), _amount);
            ...
            */
            ...
            uint IsXTokenAmount = (msg.value * IsXTokenReserve)/(ethReserve);
            require(_amount >= IsXTokenAmount, "Amount of tokens sent is less than the minimum tokens required");
            // transfer only (cryptoDevTokenAmount user can add) amount of `Crypto Dev tokens` from users account
            // to the contract
            IsXToken.transferFrom(msg.sender, address(this), IsXTokenAmount);
    ...
    */
    function removeLiquidity(uint _amount) public returns (uint , uint) {
        ...
        uint IsXTokenAmount = (getReserve() * _amount)/ _totalSupply;
        ...
        ERC20(IsXTokenAddress).transfer(msg.sender, IsXTokenAmount);
        return (ethAmount, IsXTokenAmount);
    }
    ...
    */
    function getAmountOfTokens(
        ...
        uint256 inputAmountWithFee = inputAmount * 89;
    ...
    */
    function ethToIsXToken(uint _minTokens) public payable {
        ...
        ERC20(IsXTokenAddress).transfer(msg.sender, tokensBought);
    }
    ...
    */
    function TokenToEth(uint _tokensSold, uint _minEth) public {
        ...
        ERC20(IsXTokenAddress).transferFrom(
            msg.sender,
            address(this),
            _tokensSold
        );
